...
import {ChainStore, ChainTypes} from "flashchainjs/es";
...
const MarketUtils = {
    ...
    parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/FLC, amount should be in FLC, value should be in USD, debt is in USD
                // buy is in USD, sell is in FLC
                // quote is USD, base is FLC
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in FLC/USD, amount should be in USD, value should be in FLC, debt is in USD
                // buy is in USD, sell is in FLC
                // quote is USD, base is FLC
