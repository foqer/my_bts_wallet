...
// Filter FLC 0.9.2+ import keys export file so that it will include only private keys
// that may be found in the FLC 2.0 genesis block.
// Dependencies:
// ./bloom_flashchain.dat (1,048,576 bytes) sha1 4d80faa41a5e868899febdc9dab48d1f2d567487992810cf0532f3c0ee2b266c
// nodejs, npm, and: npm install

/*
* ./bloom.dat is from flashchain/graphene/programs/genesis_util/create_bloom_filter.py
* The bloom filter was created with a genesis containing FLC prefixed keys.  Create
...
var graphenejs = require("flashchainjs");
var h = graphenejs.hash;
var key_utils = graphenejs.key;
const chainPrefix = "FLC";

fs.readFile('bloom_flashchain.dat', function (err, data) {
    if (err) throw err
    console.error('bloom_flashchain.dat (' + data.length + ' bytes)','sha1',h.sha1(data).toString('hex'),'\n')

    var bits_in_filter = data.length * 8 // 8388608 (test data)
    function in_bloom(str) {
        // echo -n '0:FLC87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3m'|sha256sum
        // str = 'FLC87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3n' // should be found
        // str = 'FLC4A43UCoWz1F5vqLxX3LLoGHKs1GzCGRjZ' // should be found
